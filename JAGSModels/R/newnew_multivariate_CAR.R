...
# Prepare data for JAGS - Quantiles
# rho = 0.95
...
#                   "abeta"=abeta,
#                   "bbeta"=bbeta)
# MCAR_model <- jags.model("bug/new_multivariate_CAR.bug", data=MCAR_data, n.chains=1, n.adapt = 0)

# Prepare data for JAGS - Log differences of quantiles
rho = 0.95
Y = c(log(sapply(data, quantile, 0.25) - sapply(data, quantile, 0.05)),
      log(sapply(data, quantile, 0.50) - sapply(data, quantile, 0.25)),
      log(sapply(data, quantile, 0.75) - sapply(data, quantile, 0.50)),
      log(sapply(data, quantile, 0.95) - sapply(data, quantile, 0.75))); names(Y) = NULL
MCAR_data <- list("Y"=Y,
                  "I"=numGroups,
                  "rho"=0.95,
                  "W"=W,
