...
for (i in 1:length(H)) {
  for (j in 1:length(rho)) {
    # Parse csv file
    file_name <- file.path(getwd(), "summary", alphabeta, sprintf("CM-H_%s-rho_%s.csv", H[i], rho[j]))
...
for (i in 1:length(H)) {
  for (j in 1:length(rho)) {
    # Parse csv file
    file_name <- file.path(getwd(), "summary", alphabeta, sprintf("meanL1-H_%s-rho_%s.csv", H[i], rho[j]))
...
for (i in 1:length(H)) {
  for (j in 1:length(rho)) {
    # Parse csv file
    file_name <- file.path(getwd(), "summary", alphabeta, sprintf("meanWAIC-H_%s-rho_%s.csv", H[i], rho[j]))
