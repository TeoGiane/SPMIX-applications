...
boundary_geometry <- function(boundary_graph, sf_geometry) {
  ...
  # Condense everything into a unique sf object
...
for (i in 1:numGroups) {
  if (dist_picker[i] == 1) {
    data[[i]] <- metRology::rt.scaled(n=100, df=6, mean=3.123188, sd=1.5)
  # } else {
    data[[i]] <- sn::rsn(n=100, xi=4, omega=1.3, alpha=-5)
...
plt_postmedian <- ggplot() +
  geom_sf(data = sf_grid, aes(fill=post_median), col='gray25', alpha = 0.6, linewidth=0.4) +
  scale_fill_gradient(low = "steelblue", high = "darkorange",
                      guide = guide_colorbar(title = "Post. Median", title.hjust = 0.5, title.position = "bottom",
                                             direction = "horizontal", barwidth = unit(3, "in"))) +
  geom_sf(data = bound_sf, fill=NA, col='darkred', linewidth=1.2) +
  theme_void() + theme(legend.position = "bottom")
# Show / Save
x11(height = 4, width = 4); plt_postmedian
pdf("plt_BDpostMean_sameMedian.pdf", height = 4, width = 4); plt_postmedian; dev.off()
...
plt_areasdens <- list(); alpha <- 0.05
for (i in 1:length(areas)) {
  df <- data.frame("x" = x,
                   "y" = colMeans(estimated_densities[[areas[i]]]))#,
                   # "ymin" = apply(estimated_densities[[areas[i]]], 2, quantile, alpha/2),
                   # "ymax" = apply(estimated_densities[[areas[i]]], 2, quantile, 1-alpha/2))
...
# Save plot
pdf("output/plt_areas_est_3_sameMedian.pdf", height = 4, width = 4); plt_areasdens[[1]]; dev.off()
pdf("output/plt_areas_est_4_sameMedian.pdf", height = 4, width = 4); plt_areasdens[[2]]; dev.off()
